package com.example.sphere.cardboard2.util;
...
import com.example.sphere.cardboard2.util.Matrix3x3d;
import com.example.sphere.cardboard2.util.Vector3d;
...
public class So3Util {
    ...
    public static void sO3FromTwoVec(Vector3d a, Vector3d b, Matrix3x3d result) {
        ...
        if(sO3FromTwoVecN.length() == 0.0D) {//如果a和b平行
            double r11 = Vector3d.dot(a, b);
            if(r11 >= 0.0D) {//如果a，b方向小于90度
                result.setIdentity();
            } else {//如果a，b方向大于90度
                Vector3d.ortho(a, sO3FromTwoVecRotationAxis);   //获得a的垂直向量sO3FromTwoVecRotationAxis
                rotationPiAboutAxis(sO3FromTwoVecRotationAxis, result);  //获得一个绕sO3FromTwoVecRotationAxis旋转的3*3的矩阵
            }
            ...
            r1.setColumn(2, temp31);            //r1 = (a,n,b叉乘n);
            ...
            r2.setColumn(2, temp31);            //r2 = (b,n,a叉乘n);
            r1.transpose();
            Matrix3x3d.mult(r2, r1, result);    //得到r2*r1'
        }
    }
